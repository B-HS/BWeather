...
export const latLonToGrid = (lat: number, lon: number): { x: number; y: number } => {
...
export const gridToLatLon = (x: number, y: number): { lat: number; lon: number } => {
    const xn = x - XO
    const yn = roPow - y + YO
    const ra = Math.sqrt(xn * xn + yn * yn)
    const raSign = snLog < 0 ? -1 : 1
